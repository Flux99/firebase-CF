...
router.post("/newuser", async (req, res) => {
    ...
    //   res.send({data: "OK"});
    const avatar = await setup_1.firestoreAsset.doc(req.body.avatarId).get();
    if (!avatar.data()) {
        console.log("No Data");
...
router.get("/verify-email", middleware_1.validateFirebaseIdToken, async (req, res) => {
    console.log(`request body is ${JSON.stringify(req.body)}`);
    //   res.send({data: "OK"});
    ...
    //   // response.success(res, 302, message.ASSET_FOUND, avatar.data());
    // }
    const auth = await auth_1.getAuth();
    await auth_1.sendEmailVerification(auth.currentUser)
        .then(() => {
